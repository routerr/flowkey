...
use kms_config::Config;
use kms_core::daemon::{DaemonRuntime, DaemonState};
use kms_core::recovery::ReconnectBackoff;
use kms_core::DaemonCommand;
use kms_core::DaemonStatus;
use kms_input::capture::{CaptureSignal, InputCapture};
use kms_input::hotkey::HotkeyBinding;
use kms_input::loopback::{LoopbackSuppressor, SharedLoopbackSuppressor};
use kms_input::InputEventSink;
use kms_net::connection::{
    connect_and_authenticate, run_authenticated_session, session_channel, SessionSender,
};
use kms_net::heartbeat::HeartbeatConfig;
...
pub async fn run_daemon(config: Config) -> Result<()> {
    ...
    let hotkey_binding = match HotkeyBinding::parse(&config.switch.hotkey) {
        ...
        Err(error) => {
            ...
            HotkeyBinding {
                code: kms_input::keycode::KeyCode::Character('k'),
                modifiers: kms_input::event::Modifiers {
                    shift: true,
                    control: true,
                    alt: true,
                    meta: false,
    ...
    let incoming_task = tokio::spawn(async move {
        loop {
            ...
            tokio::spawn(async move {
                match kms_net::connection::authenticate_incoming_stream(&config, stream).await {
                    Ok(connection) => {
...
fn spawn_hotkey_watcher(
    ...
    let mut capture: Box<dyn InputCapture> = Box::new(
        kms_platform_macos::capture::MacosCapture::with_loopback(binding, Some(loopback)),
    );

    #[cfg(target_os = "windows")]
    let mut capture: Box<dyn InputCapture> = Box::new(
        kms_platform_windows::capture::WindowsCapture::with_loopback(binding, Some(loopback)),
    );
            ...
            match capture.poll() {
                ...
                Some(CaptureSignal::Input(event)) => {
                    let active_peer_id = {
                        ...
                        if matches!(
                            runtime.state,
                            kms_core::daemon::DaemonState::Controlling { .. }
                        ) {
...
fn create_platform_input_sink(
    ...
    #[cfg(target_os = "macos")]
    {
        match kms_platform_macos::inject::MacosInjector::with_loopback(Some(loopback)) {
    ...
    #[cfg(target_os = "windows")]
    {
        match kms_platform_windows::inject::WindowsInjector::with_loopback(Some(loopback)) {
            Ok(injector) => (Box::new(injector), "native", None),
            Err(error) => {
...
fn platform_notes() -> Vec<String> {
    #[cfg(target_os = "macos")]
    {
        return kms_platform_macos::permissions::PermissionStatus::probe().notes();
    }

    #[cfg(target_os = "windows")]
    {
        return kms_platform_windows::permissions::PermissionStatus::probe().notes();
    }
...
impl InputEventSink for LoggingInputSink {
    fn handle(&mut self, event: &kms_input::event::InputEvent) -> Result<(), String> {
...
mod tests {
    ...
    use kms_core::daemon::{DaemonRuntime, DaemonState};
    use kms_core::status::DaemonStatus;
    use kms_input::event::InputEvent;
    use kms_input::InputEventSink;
    use kms_net::connection::session_channel;
    ...
    fn temp_status_path(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "kms-daemon-bootstrap-{label}-{}-{}.toml",
            std::process::id(),
            SystemTime::now()
