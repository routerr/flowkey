...
impl Config {
    ...
    pub fn default_path() -> Result<PathBuf> {
        if let Ok(path) = env::var("KMS_CONFIG") {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            return Ok(PathBuf::from(app_data).join("kms").join("config.toml"));
        }
...
mod tests {
    ...
    use super::{
        control_path_for_config_path, normalize_id, status_path_for_config_path, Config, PeerConfig,
    };
    ...
    fn status_path_follows_config_location() {
        let config_path = PathBuf::from("/tmp/kms/config.toml");

        let status_path = status_path_for_config_path(&config_path);

        assert_eq!(status_path, PathBuf::from("/tmp/kms/status.toml"));
    }
    ...
    fn control_path_follows_config_location() {
        ...
        assert_eq!(control_path, PathBuf::from("/tmp/kms/control.toml"));
    }
}
