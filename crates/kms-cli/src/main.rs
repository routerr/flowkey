...
use kms_core::{DaemonCommand, DaemonStatus};
use kms_crypto::{HandshakeOffer, NodeIdentity};
use kms_daemon::run_daemon;
use std::path::Path;
use tracing::info;
use tracing::warn;
...
#[derive(Debug, Parser)]
#[command(name = "kms", version, about = "LAN keyboard/mouse sharing daemon")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
