...
use kms_config::{Config, PeerConfig};
use kms_crypto::{NodeIdentity, SessionChallenge, SessionResponse};
use kms_input::event::InputEvent;
use kms_protocol::message::{
...
pub async fn run_authenticated_session(
    ...
    sink: &mut dyn kms_input::InputEventSink,
            ...
            result = tokio::time::timeout(Duration::from_secs(heartbeat.timeout_secs), read_message(stream)) => {
                ...
                match message {
                    ...
                    Message::InputEvent { sequence, event } => {
                        ...
                        kms_net_route_input_event(sink, &event)?;
                    }
...
mod tests {
    ...
    use kms_config::{Config, PeerConfig};
    use kms_crypto::SessionChallenge;
    use kms_input::event::{InputEvent, Modifiers};
    use tokio::net::{TcpListener, TcpStream};
    ...
    impl kms_input::InputEventSink for NoopSink {
        fn handle(&mut self, _event: &kms_input::event::InputEvent) -> Result<(), String> {
            Ok(())
        }
    ...
    async fn tcp_handshake_authenticates_bidirectionally() {
        let mut server_config = Config::load_or_create().expect("server config should exist");
        let mut client_config = Config::load_or_create().expect("client config should exist");

        server_config.node.id = "server-node".to_string();
        server_config.node.name = "Server Node".to_string();
        client_config.node.id = "client-node".to_string();
        client_config.node.name = "Client Node".to_string();
